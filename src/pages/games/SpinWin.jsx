...
const SEGMENTS = [
  { tier: "loss", color: "#cc2222", img: null, text: "" },
  {
    tier: "1000",
    color: "#00ff88",
    img: "https://images.unsplash.com/photo-1554768804-50c1e2b50a6e?w=120&q=70&fm=jpg",
    text: "$1K",
  },
  { tier: "loss", color: "#cc2222", img: null, text: "" },
  { tier: "prize", color: "#c084fc", img: null, text: "" },
  { tier: "loss", color: "#cc2222", img: null, text: "" },
  {
    tier: "5000",
    color: "#00d4ff",
    img: "https://images.unsplash.com/photo-1601597111158-2fceff292cdc?w=120&q=70&fm=jpg",
    text: "$5K",
  },
  { tier: "loss", color: "#cc2222", img: null, text: "" },
  {
    tier: "10000",
    color: "#f0c040",
    img: "https://images.unsplash.com/photo-1518458028785-8fbcd101ebb9?w=120&q=70&fm=jpg",
    text: "$10K",
  },
  { tier: "loss", color: "#cc2222", img: null, text: "" },
  {
...
async function drawWheel(canvas) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    // Fill background
    ctx.fillStyle = isLoss ? (i % 2 === 0 ? "#1c0404" : "#220606") : "#0a1a0a";
    ctx.fill();

    // For cash segments — draw money image as background
    if (!isLoss && seg.tier !== "prize" && seg.img) {
      try {
        const img = await loadImage(seg.img);
        if (img) {
          // Find segment center point
          const labelR = R * 0.67;
          const lx = cx + labelR * Math.cos(midRad);
          const ly = cy + labelR * Math.sin(midRad);
          const size = 45;
          ctx.globalAlpha = 0.5; // semi-transparent so color shows through
          ctx.drawImage(img, lx - size / 2, ly - size / 2, size, size);
          ctx.globalAlpha = 1;
        }
      } catch {
        // Ignore image loading failures for decorative wallets/images.
      }
    }
    ...
    if (isLoss) {
      ...
      // No text — image only. Amount hidden from user.
    }
    ctx.restore();
  }
...
export default function SpinWin() {
  ...
  useEffect(() => {
    const init = async () => {
      await drawWheel(canvasRef.current);
      // Redraw after 1s to catch any late-loading images
      setTimeout(() => drawWheel(canvasRef.current), 1000);
      setTimeout(() => drawWheel(canvasRef.current), 2500);
    };
    init();
    fetchBalance();
  }, []);
      ...
      <div style={{ maxWidth: "1060px", margin: "0 auto" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: "0.85rem" }}>
            Min $100 · 70% house edge · Provably fair · Physical prizes
            available
          </p>
        </div>
